...
void Queue_Init( Queue *queue ) {
    if (queue == NULL) {  //when the queue is empty, call the error function
        Queue_Error(QERR_INIT);
    } else {
        for (int i = 0; i < QUEUE_SIZE; i++) {  //set all values in the array to '*'
            queue->array[i] = '*';
        }
        queue->firstIndex = 0;  //set the first index to 0
        queue->freeIndex = 0;   //set the free index to 0
    }
}
...
int nextIndex( int index ) {
    return (index + 1) % QUEUE_SIZE;  //return the next index in the array
}
...
int Queue_IsEmpty( const Queue *queue ) {
    return (queue->firstIndex == queue->freeIndex)? 1 : 0;  //return 1 if the queue is empty, 0 otherwise
}
...
int Queue_IsFull( const Queue *queue ) {
    return (nextIndex(queue->freeIndex) == queue->firstIndex) ? 1 : 0;  //return 1 if the queue is full, 0 otherwise
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
    if (Queue_IsEmpty(queue)) {  //if the queue is empty, call the error function
        Queue_Error(QERR_FRONT);
    } else {
        *dataPtr = queue->array[queue->firstIndex];  //return the first element in the queue
    }
}
...
void Queue_Remove( Queue *queue ) {
    if (Queue_IsEmpty(queue)) {  //if the queue is empty, call the error function
        ...
        queue->firstIndex = nextIndex(queue->firstIndex);  //remove the first element in front of the queue
    }
}
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
    if (Queue_IsEmpty(queue)) {  //if the queue is empty, call the error function
        ...
        Queue_Front(queue, dataPtr);  //read the first element from the queue
        Queue_Remove(queue);  //remove the first element in front of the queue
    }
}
...
void Queue_Enqueue( Queue *queue, char data ) {
    if (Queue_IsFull(queue)) {  //if the queue is full, call the error function
        Queue_Error(QERR_ENQUEUE);
    } else {
        queue->array[queue->freeIndex] = data;  //insert the element into the queue
