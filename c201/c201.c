...
void List_Init( List *list ) {
    list->activeElement = NULL;     //initialization of the list structure
...
void List_Dispose( List *list ) {
    ListElementPtr element = list->firstElement;    //temporary pointer to the first element
    while (element != NULL) {   //cycle for disposing all elements
        ListElementPtr nextElement = element->nextElement;
        free(element);  //freeing the memory
        element = nextElement;
    }
    list->firstElement = NULL;  //setting the list to the initial state
...
void List_InsertFirst( List *list, int data ) {
    ListElementPtr element = (ListElementPtr) malloc(
            sizeof(struct ListElement));    //allocating memory for the element
    if (element == NULL) {   //checking if the memory allocation was successful
        ...
        list->firstElement = element;   //setting the element as the first element
    }
}
...
void List_DeleteFirst( List *list ) {
    if (list->firstElement == NULL);    //if list is empty nothing happens
    else {
        if (list->activeElement == list->firstElement) {    //if the first element is active, activity is lost
            list->activeElement = NULL;
        }
        ListElementPtr element = list->firstElement;    //temporary pointer to the element
...
void List_DeleteAfter( List *list ) {
    if (list->activeElement == NULL || list->activeElement->nextElement ==
                                       NULL);   //if the list is not active or the active element is the last one, nothing happens
    else {
        ListElementPtr element = list->activeElement->nextElement;    //temporary pointer to the element
        list->activeElement->nextElement = list->activeElement->nextElement->nextElement;    //setting the element as the next element
        free(element);   //freeing the memory
    }
}
...
void List_InsertAfter( List *list, int data ) {
    if (list->activeElement == NULL);   //if the list is not active nothing happens
        ...
        if (element == NULL) {   //checking if the memory allocation was successful
            ...
            list->activeElement->nextElement = element;    //setting the element as the next element
        }
    }
}
...
void List_GetValue( List *list, int *dataPtr ) {
    if (list->activeElement == NULL) {   //checking if the list is active
        List_Error();
    } else {
        *dataPtr = list->activeElement->data;   //storing the value in to the address
    }
}
...
void List_SetValue( List *list, int data ) {
    if (list->activeElement == NULL);   //if the list is not active nothing happens
        ...
        list->activeElement->data = data;   //setting the data
    }
}
...
void List_Next( List *list ) {
    if (list->activeElement == NULL);   //if the list is not active nothing happens
        ...
        list->activeElement = list->activeElement->nextElement;   //setting the next element as the active one
    }
}
